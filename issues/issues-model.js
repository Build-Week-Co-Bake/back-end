...
function findById(id) {
  return db('issues')
    .where('issues.id', id)
    .join('users', 'users.id', 'issues.user_id')
    .select('issues.id', 'issues.title', 'issues.description', 'issues.city', 'issues.hoa', 'issues.image', 'issues.upvotes', 'issues.created_on', 'users.profile_pic', 'users.name', 'users.email')
}
